...
#define topCountRef 50 //reference with period: Tref = 2*topCountRef*sampling
//Reference setting: comment one at a time in order to choose between square or step reference
#define refPos 0.5f //positive and negative value of reference, change this value if you
#define refNeg -0.5f
//believe appropriate
#define squareSig 1
#define stepSig 2
...
#define Kp 3.69 //Proportional gain
#define Ki 1.94 //Integral gain
#define Kd 0.207 //Derivative gain
#define eCumTop 100.0 //set it based on experience
#define tSampling 10.048e-3; //sampling time, change it when time-base for overflow changes

void calculateControl(void)
{
  //u = r; //reference on the input, used for system identification. Uncomment this line for identifying
         //comment it once the control has been developed
       
  e = r-x1; //current error
  eCum = e + eCumOld; //cumulative error
  
  if (eCum > eCumTop) //anti-windup
    eCum = eCumTop;
  else if (eCum < -eCumTop)
    eCum = -eCumTop;

  //derivative of error (backguards) 
  eDot = (e - eOld)/tSampling;
  
  //integral of cumulative error
  eInt = eCum*tSampling; //one way to integrate
  //eInt = ((eCum + eCumOld)/2)*tSampling;//a slightly more accurate way to integrate (average approach)
 
  eOld = e; //old error, the next iteration
  eCumOld = eCum; //old cumulative error, the next iteration

  u = Kp*e + Ki*eInt + Kd*eDot; //PID control action  
